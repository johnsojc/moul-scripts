...
parts covered by the terms of the Bink SDK EULA, 3ds Max EULA,
...
sdlS1FinaleBahro = ["islmS1FinaleBahro", "islmS1FinaleBahroCity1",
                    "islmS1FinaleBahroCity2", "islmS1FinaleBahroCity3",
                    "islmS1FinaleBahroCity4", "islmS1FinaleBahroCity5",
                    "islmS1FinaleBahroCity6"]
pagesS1FinaleBahro = ["bahroFlyers_arch", "bahroFlyers_city1",
                      "bahroFlyers_city2", "bahroFlyers_city3",
                      "bahroFlyers_city4", "bahroFlyers_city5",
                      "bahroFlyers_city6"]
# S1FinaleBahro = []
...
class city(ptResponder):
    ...
    def __init__(self):
        ...
        ## NOT USING THIS FOR NOW - MAY NEED TO LOAD IN SPECIFIC PAGE(S)
        ## FOR FUTURE CITY AREAS...
        # if spTitle == "KadishGallery":
        #     print ("city.__init__(): we're linking into KadishGallery, "
        #            "only that page will be loaded")
        #     IsKadishGallery = 1
        # else:
        #     print ("city.__init__(): we're NOT linking into KadishGallery, "
        #            "will load the entire city")

        pages = []

        # Add the common pages
        # actually, we'll just set these to load
        #     automatically in the .age file
        # pages += ["KadishGallery""]

        # For the non-public age, add all the remaining pages
        if not IsKadishGallery:
            pages += ["canyon", "cavetjunction", "courtyard",
                      "ferry", "greatstair", "guildhall",
                      "harbor", "HarborReflect"]
            pages += ["islmGreatZeroState", "islmJCNote",
                      "islmNegilahnCreatureChartGUI",
                      "islmNickNote", "islmPodMapGUI"]
            pages += ["islmWatsonLetterGUI", "KahloPub",
                      "kahlopubtunnel", "library", "LibraryInterior"]
            pages += ["MuseumInteriorPage", "palace", "trailerCamPage"]
            pages += ["islmBahroShoutFerry", "islmBahroShoutLibrary",
                      "islmBahroShoutPalace"]
            #pages += ["islmDRCStageState01", "islmDRCStageState02",
            #          "islmDRCTentTablePic", "islmFanSoundRun"]
            #pages += ["islmLibBanners00Vis", "islmLibBanners02Vis",
            #          "islmLibBanners03Vis"]
            #pages += ["LibraryAhnonayVis", "LibraryErcanaVis",
            #          "LibraryGarrisonVis", "LibraryKadishVis",
            #          "LibraryTeledahnVis"]
...
#            ageSDL["islmKadishGalleryDoorVis"] = (1,)
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        ageSDL = PtGetAgeSDL()
        PtDebugPrint("city.OnSDLNotify():\t VARname: %s, "
...
#                return
    ...
    def ILoadS1FinaleBahro(self, bahro, state):
        print ("city.ILoadS1FinaleBahro(): bahro = %d, load = %d"
               % (bahro, state))
#        if not self.sceneobject.isLocallyOwned():
#            return
        if state:
