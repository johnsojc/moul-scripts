...
class ahnyQuabs(ptModifier, object):
    ...
    last_update = property(_last_update_get,
                           doc="Gets the last time a quab was killed/born")
    ...
    quabs = property(
        _quabs_get, _quabs_set, doc="Gets the number of quabs alive")

    def OnServerInitComplete(self):
        PtDebugPrint("ahnyQuabs.OnServerInitComplete():\tWhen I got here...",
                     level=kWarningLevel)
        PtDebugPrint("ahnyQuabs.OnServerInitComplete():\t... "
                     "there were already %i quabs"
                     % (self.quabs), level=kWarningLevel)
        self.brains = PtGetAIAvatarsByModelName(kQuabAvatarName)

        # Sanity Check: Before we think about doing any processing,
        #               make sure there are no quabs already loaded.
        #               We may have arrived after the last man left
        #               but before the server shut down. Therefore,
        #               we will already have quabs... So we don't want
        #               to spawn another 20 or so dupe avatar clones.
        if len(self.brains) != 0:
            PtDebugPrint("ahnyQuabs.OnServerInitComplete():\t... "
                         "and they were already spawned!",
                         level=kWarningLevel)
        ...
        if self.sceneobject.isLocallyOwned():
            ...
            if toSpawn:
                PtDebugPrint("ahnyQuabs.OnServerInitComplete():\t... "
                             "and I need to spawn %i more"
                             % (toSpawn), level=kWarningLevel)
                self.quabs += toSpawn
            ...
            # Shuffle the spawn points around so we don't get the same
            # quabs appearing every single time. That would be quite boring.
            qSpawns = list(quabObjects.value)
            random.shuffle(qSpawns)

            # On quab spawning...
            # We will load the avatar clones manually
            #              if we are the first one in.
            # We will obtain the ptCritterBrains in an OnAIMsg callback.
            for i in xrange(self.quabs):
                PtLoadAvatarModel(kQuabAvatarName, qSpawns[i].getKey(),
                                  "Quab %i" % i)
    ...
    def OnAIMsg(self, brain, msgType, userStr, args):
        if msgType == PtAIMsgType.kBrainCreated:
            ...
            PtDebugPrint("ahnyQuabs.OnAIMsg():\t%s created"
                         % (userStr), level=kDebugDumpLevel)
        ...
        if msgType == PtAIMsgType.kArrivedAtGoal:
            ...
            PtDebugPrint("ahnyQuabs.OnAIMsg():\t%s arrived at goal"
                         % (userStr), level=kDebugDumpLevel)
            return

    def OnNotify(self, state, id, events):
        if id == deadZone.id:
            # Make sure this isn't the player jumping
            #         into the water for a quick swim
            # Musing: Ideally, we would despawn the clone here since it's
            #         now useless, but removing the brain without causing
            #         rampant issues might be problematic...
            if PtFindAvatar(events) != PtGetLocalAvatar():
                self.quabs -= 1
                PtDebugPrint("ahnyQuabs.OnNotify():\tQuabs remaining: %i"
                             % (self.quabs), level=kWarningLevel)
                return
    ...
    def OnUpdate(self, seconds, delta):
        for brain, name in self.brains:
            ...
            # If you think running this every frame is a bad idea,
            # remember how badly the quab AI sucked in MOUL.
            # This won't kill you.
    ...
    def _Think(self, brain, name):
        ...
        # in a straight line away from whatever is chasing them.
        # Fun fact: alligators can catch dogs because they do the same thing.
        # The mammalian adaptation is the ability to turn quickly
        # (which the dog does not actually do)...
        # This evolutionary biology lesson is thanks to Hoikas,
        # the Mammalogy drop-out
        monsters = brain.playersICanHear()
        if len(monsters) == 0:
            if running:
                PtDebugPrint("ahnyQuabs._Think():\t%s is now safe."
                             % (name), level=kDebugDumpLevel)
        ...
        endPos = (ptPoint3(curPos.getX() + runaway.getX(),
        ...
        if not running:
            ...
            PtDebugPrint("ahnyQuabs._Think():\tTime for %s to run away!"
                         % name, level=kDebugDumpLevel)
    ...
    def _IsRunningAway(self, brain):
        ...
        if brain.runningBehavior(brain.idleBehaviorName()):
            return False
        raise RuntimeError("Quab brain running neither the idle nor "
                           "the run behavior. WTF?")

    def _RunAway(self, brain, runAway=True):
        # Quick helper because I'm lazy and
        # the behavior apis are really stupid
        if runAway:
