...
# March 2003
This script now also provides the GUI for books taken off of the Personal
Age Bookshelf. There are a few differences between books on pedestals vs.
books on the psnlBookshelf:
- The "Share Book" decal on the book itself is never available on a book
  from the psnlBookshelf
- Turned corners, indicating more pages in the book, are only available
  in books in the psnlBookshelf
...
BookNumber = 0  # which book it is on the shelf.
                # The Neighborhood book currently is 0.
                # The Teledahn Book is currently 2.
CurrentPage = 1  # The last page the book was opened to. Read from an SDL.

# the global ptBook object... there can only be one book displayed at one time,
# so only one global needed (hopefully)
gLinkingBook is None
NoReenableBook = 0
...
class ahnyLinkBookGUIPopup(ptModifier):
    ...
    def OnServerInitComplete(self):
        # only in the personal age should actBookshelf be anything,
        # so this should only happen in the personal age
        if actBookshelf:
            ageSDL = PtGetAgeSDL()
            ...
            for event in events:
                if (event[0] == kMultiStageEvent and
                    ...
                    print ("ahnyLinkBookGUIPopup: "
                           "attempting to draw link panel gui")
                    self.IShowBookNoTreasure()
                    OfferedBookMode = false
                    BookOfferer = None
                ...
                if event[0] == PtEventType.kBook:
                    ...
                    if event[1] == PtBookEventTypes.kNotifyImageLink:
                        if (event[2] >= xLinkingBookDefs.kFirstLinkPanelID or
                            ...
                            # assume this is a normal pedestal book,
                            # (i.e. not the psnlBookshelf)
                            # Run the responder indicated in Max
                            ...
                            if resetSDL:
                                ...
                                for i in iown.getChildNodeRefList():
                                    ...
                                    if info:
                                        ...
                                        if name == "Ahnonay":
                                            # or name == "AhnySphere01" or
                                            # name == "AhnySphere02" or
                                            # name == "AhnySphere03" or
                                            # name == "AhnySphere04":
                                            # its not working if we reset all
                                            # of the sdl here
                                            # so we'll just create some
                                            # chronicle vars and reset the
                                            # sdl in the age
                                            print (
                                                "attempting to reset sdl for",
                                                name)
                                            asdl = info.getAgeSDL()
                            ...
                            if (respNum == 1 or respNum == 0):
                                respLinkSphere01.run(
                                    self.key,avatar=PtGetLocalAvatar())
                            elif (respNum == 2):
                                respLinkSphere02.run(
                                    self.key,avatar=PtGetLocalAvatar())
                            elif (respNum == 3):
                                respLinkSphere03.run(
                                    self.key,avatar=PtGetLocalAvatar())
                            elif (respNum == 4):
                                respLinkSphere04.run(
                                    self.key,avatar=PtGetLocalAvatar())
                            else:
                        ...
                        if CurrentPage > 1:
                            PtDebugPrint("ahnyLinkBookGUIPopup: "
                                         "going to page %d (ptBook page %d)"
                                         % (CurrentPage, (CurrentPage-1)*2),
                                         level=kDebugDumpLevel)
                            gLinkingBook.goToPage((CurrentPage-1)*2)

                    elif event[1] == PtBookEventTypes.kNotifyHide:
                        PtDebugPrint("ahnyLinkBookGUIPopup:Book: "
                                     "NotifyHide",
                                     level=kDebugDumpLevel)
    ...
    def IShowBookNoTreasure(self):
        ...
        try:
            ...
            print ("ahnyLinkBookGUIPopup: "
                   "could not find age Ahnonay's linking panel")

        '''
        showOpen = 0
        #If this is a normal pedestal book, the panel is defined
        #in the Max GUI. If this is the personal Age Bookshelf,
        #the panel is passed in a note from psnlBookshelf.
        #Determine the source here.
        ...
        if agePanel:
            try:
                ...
                if sharable:
                    PtDebugPrint(
                        "ahnyLinkBookGUIPopup: "
                        "%s's book definition can't be shared"
                        % (agePanel),level=kErrorLevel)
                else:
                ...
                PtDebugPrint("ahnyLinkBookGUIPopup: "
                             "could not find age %s's linking panel"
                             % (agePanel),level=kErrorLevel)
        else:
            PtDebugPrint("ahnyLinkBookGUIPopup: "
                        "no age link panel"
    ...
    def OnTimer(self, id):
        ...
        global kGrsnTeamBook
        if id == kGrsnTeamBook:
            print ("\nahnyLinkBookGUIPopup.OnTimer:Got timer callback. "
                   "Removing popup for a grsn team book.")
            gLinkingBook.hide()
