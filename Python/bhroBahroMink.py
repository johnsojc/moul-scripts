...
class bhroBahroMink(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        if psnlSDL["psnlBahroWedge11"][0]:
            print ("bhroBahroMink.OnServerInitComplete: "
                   "You have the Minkata wedge, no need to display it.")
            respRing.run(self.key, fastforward=1)

    ###########################
    def OnNotify(self, state, id, events):
        # print ("bhroBahroMink.OnNotify: state=%s id=%d events="
        #        % (state, id), events)
        ...
        if id == clickable.id and not state:
            ...
            if not sdlVal:
                print ("bhroBahroMink.OnNotify:  "
                       "Turning wedge SDL of psnlBahroWedge11 to On")
