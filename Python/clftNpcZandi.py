...
combination shall include the source code for the parts of OpenSSL and
IJG JPEG Library used as well as that of the covered work.
...
respWindmillNotTurning = ptAttribResponder(11, "Windmill not turning",
                                           ["1", "2"])
...
respZandiSayings = ptAttribResponder(21, "Zandi sayings",
                                     ["welcome", "dryheat", "pizza",
                                      "keepexploring", "interrupt",
                                      "steak", "welcome2"])
...
jcDict = {JC.Trailer: 'c', JC.Imager: 'd', JC.Bedroom: 'e', JC.Wahrk: 'b',
          JC.Sign: 'a', JC.Bucket: 'g', JC.Door: 'f'}
...
class clftNpcZandi(ptModifier):
    ...
    def OnNotify(self, state, id, events):
        ...
        if id == NpcSpawner.id:  # Causes Zandi to Idle
                                 # even before avatar visits
            ...
            # So, only let the avatar who enters do this.
            # Zandi != local avatar, so win
            if PtFindAvatar(events) != PtGetLocalAvatar():
                return

            for event in events:
                if event[0] == 1 and event[1] == 1:  # avatar physically
                                                     # approached Zandi
                    self.NearZandi = 1
                    if not (self.IsTalking or self.IsIgnoring):
                        self.ZandiSpeaks()

                elif event[0] == 1 and event[1] == 0:  # avatar physically
                                                       # stepped away
                                                       # from Zandi
                    #~ print "Stepped away"
                    self.NearZandi = 0
            ...
            for event in events:
                if event[0] == 10 and event[2] == 3:  # A Zandi behavior just
                                                      # finished. Returning to
                                                      # idle animation.
                    print "zandi is done doing a behavior"
                    self.DoingBehavior = 0
                    break

        elif id in (respBrakeNotReleased.id, respWindmillNotTurning.id,
                    respVisionNotSeen.id, respNoTrailerJC.id,
                    respNoImagerJC.id, respNoBedroomJC.id,
                    respNoWahrkJC.id, respNoSignJC.id,
                    respNoBucketJC.id, respNoDoorJC.id,
                    respDoorNotOpen.id, respZandiSayings.id):
    ...
    def OnTimer(self, id):
        ...
        if id < 10:
            ...
            if not self.DoingBehavior:
                ...
                MultiStage01.gotoStage(self.NpcName, id, dirFlag=1,
                                       isForward=1)
            if isinstance(self.ZandiFace, type("")):
            ...
            if not self.DoingBehavior:
                print "do turn page"
                self.DoingBehavior = 1
                MultiStage01.gotoStage(self.NpcName, 2, dirFlag=1,
                                       isForward=1)  # turn page
                PtAtTimeCallback(self.key, PageTurnInterval, TimerID.TurnPage)
    ...
    def HaventSeenImagerMessage(self):
        ...
        entry = vault.findChronicleEntry("YeeshaVisionViewed")
        if isinstance(entry, type(None)):
            PtDebugPrint("ERROR: clftNpcZandi.HaventSeenImagerMessage: "
                         "cannot find YeeshaVisionViewed chronicle entry")
            return 1
    ...
    def PlaySecondWelcome(self):
        # we're not actually playing the second welcome,
        # but I don't want to break the logic
    ...
    def ZandiSpeaks(self, clicked=0):
        ...
        if self.NeedsWelcome(clicked):
            ...
            if not self.CheckForJC(jcProgress, JC.Trailer):
            ...
            elif (not self.CheckForJC(jcProgress, JC.Wahrk) and not
                  PtIsDemoMode()):
            ...
            elif (not self.CheckForJC(jcProgress, JC.Sign) and not
                  PtIsDemoMode()):
                ...
                self.LastSpeech = 11
                sayings = ["welcome", "welcome2", "dryheat", "keepexploring",
                           "steak", "pizza", "interrupt"]
