...
class xKI(ptModifier):
    ...
    def __init__(self):
        ...
        PtDebugPrint(u"__init__: xKI: v{}".format(self.version))
    ...
    def __del__(self):
        ...
        if PtGetAgeName() == "Jalak":
            PtDebugPrint(u"__del__:  xKI: Unloading Jalak GUI dialog.", level=kWarningLevel)
    ...
    def OnAccountUpdate(self, updateType, result, playerID):
        ...
        if updateType == PtAccountUpdateType.kActivePlayer and result == 0 and playerID != 0:
            PtDebugPrint(u"xKI.OnInit():  Active player set. Clear to re-init KI GUI.", level=kDebugDumpLevel)
            self.KIGUIInitialized = False
    ...
    def OnServerInitComplete(self):
        ...
        if self.markerGameManager is None:
            ...
            PtDebugPrint(u"xKI.OnInit():  Could not find marker manger, re-creating a new one.", level=kErrorLevel)
            ...
            if ageName.lower() != "startup":
                PtDebugPrint(u"xKI.OnInit():  Reloading Marker Game Manager.", level=kDebugDumpLevel)
                self.markerGameManager = MarkerGameManager(self)
        ...
        PtDebugPrint(u"xKI.OnInit():  Age = {}".format(ageName), level=kDebugDumpLevel)

        # Set up Jalak GUI.
        if ageName == "Jalak":
            PtDebugPrint(u"xKI.OnInit():  Loading Jalak GUI dialog.", level=kWarningLevel)
    ...
    def OnKIMsg(self, command, value):
        ...
        if command == kEnterChatMode and not self.KIDisabled:
            ...
            if "<p>" in value:
                ...
                PtDebugPrint(u"xKI.OnKIMsg():  Pellet Imager: {}".format(self.pelletImager), level=kDebugDumpLevel)
                return
            try:
    ...
    def CheckKILight(self):
        ...
        if not timeRemaining:
            ...
            PtDebugPrint(u"xKI.CheckKILight():  No KI light.", level=kDebugDumpLevel)
    ...
    def DoKILight(self, state, ff, remaining=0):
        ...
        if len(respList) > 0:
            ...
            for resp in respList:
                PtDebugPrint(u"xKI.DoKILight():    resp = {}".format(resp.getName()))
                if resp.getName() == thisResp:
                    ...
                    if state:
                        ...
                        PtDebugPrint(u"xKI.DoKILight():  Light was on in previous age, turning on for remaining {} seconds.".format(remaining), level=kWarningLevel)
                        curTime = PtGetDniTime()
                ...
                if myAge is not None:
                    if myAge.getAgeFilename() == "Neighborhood":
                        ...
                        if configName not in self.BKConfigFolderDict:
                            ...
                            PtDebugPrint(u"xKIBigKIRefreshAgesOwnedFolder():  adding owner config for Age {}.".format(GetAgeName(myAge)), level=kDebugDumpLevel)
    ...
    def ProcessNotifySettingsExpanded(self, control, event):
        ...
        if event == kShowHide:
            ...
            if kiID == kGUI.BKIKIFontSize:
                ...
                if self.chatMgr.ticksOnFull == kChat.FadeTimeMax:
                    self.chatMgr.fadeEnableFlag = False
                    PtDebugPrint(u"xKI.ProcessNotifySettingsExpanded():  FadeTime disabled.", level=kDebugDumpLevel)
                else:
                    self.chatMgr.fadeEnableFlag = True
                    PtDebugPrint(u"xKI.ProcessNotifySettingsExpanded():  FadeTime enabled.", level=kDebugDumpLevel)
