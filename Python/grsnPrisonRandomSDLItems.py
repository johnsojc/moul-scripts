...
class grsnPrisonRandomSDLItems(ptResponder):
    ...
    def __init__(self):
        ...
        PtDebugPrint("__init__: grsnPrisonRandomItems v{}",format(self.version))
        random.seed()
    ...
    def IManageYeeshaPage(self):
        ...
        if vault:  # is the Vault online?
            ...
            if entry is None:
                ...
                PtDebugPrint("grsnPrisonRandomSDLItems.IManageYeeshaPage():  This is your first visit to the Prison. Updated Chronicle.")

            else:
                PtDebugPrint("grsnPrisonRandomSDLItems.IManageYeeshaPage():  grsnPrisonRandomItems: You've been to the Prison before.")

                chance = random.random()
                ageSDL = PtGetAgeSDL()
                if chance > (1-kChanceOfYP):
                    ageSDL["grsnYeeshaPage02Vis"] = (1,)
                    PtDebugPrint("grsnPrisonRandomSDLItems.IManageYeeshaPage():  A YP is here.")
                else:
                    ageSDL["grsnYeeshaPage02Vis"] = (0,)
                    PtDebugPrint("grsnPrisonRandomSDLItems.IManageYeeshaPage():  A YP is NOT here.")

                ageSDL.sendToClients("grsnYeeshaPage02Vis")

        else:
            PtDebugPrint("grsnPrisonRandomSDLItems.IManageYeeshaPage():  Error trying to access the Chronicle.")
