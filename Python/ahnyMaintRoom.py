...
class ahnyMaintRoom(ptResponder):
    ...
    def __init__(self):
        ...
        PtDebugPrint("__init__: ahnyMaintRoom v{}".format(self.version))
    ...
    def OnFirstUpdate(self):
        ...
        try:
            ...
            PtDebugPrint("ahnyMaintRoom.OnFirstUpdate():  ERROR: Cannot find the Ahnonay Age SDL")
        ...
        if SphereNum.value == 4:
            ...
            if SphereNum.value != 1 and SphereNum.value != 2 and SphereNum.value != 3:
                PtDebugPrint("ahnyMaintRoom.OnFirstUpdate():  ERROR: Invalid sphere# set in component.  Disabling clickable.")
    ...
    def OnTimer(self, id):
        if id == 1:
            ...
            if actingAvatar == PtGetLocalAvatar():
                ...
                PtDebugPrint("ahnyMaintRoom.OnTImer():  advanced from sphere {} with maintainence button".format(ageSDL["ahnyCurrentSphere"][0]))
                PtDebugPrint("ahnyMaintRoom.OnTImer():  sphere {} will now be the active sphere".format(SphereNum.value))
                if SphereNum.value == 4:
                    ageSDL["ahnyImagerSphere"] = (SphereNum.value,)
                    boolHubDoor = ageSDL["ahnyHubDoor"][0]
                    if boolHubDoor and ageSDL["ahnyCurrentSphere"][0] != 4:
                        PtDebugPrint("ahnyMaintRoom.OnTImer():  Door is open and we're not going to Sphere 4, so close it.")
                        ageSDL["ahnyHubDoor"] = (0,)
                    elif not boolHubDoor and ageSDL["ahnyCurrentSphere"][0] == 4:
                        PtDebugPrint("ahnyMaintRoom.OnTImer():  Door is not open and we're going to Sphere 4, so open it.")
        ...
        if id == ActAdvanceSwitch.id and state:
            ...
            if diffsphere == 0:
                ...
                if diffsphere == 1:
                    ...
                    PtDebugPrint("ahnyMaintRoom.OnNotify():  ERROR: Sphere advancement# not possible??")
    ...
    def SphereDifference(self):
        ...
        PtDebugPrint("ahnyMaintRoom.SphereDifference():  Setting sphere difference for Maint Room switch to {}".format(diffsphere))
