...
imagerRespN = ptAttribResponder(8, "N Responder",
                                ['State 1', 'State 2', 'State 3',
                                 'State 4', 'SOLVED', 'State 6', 'State 7'])
imagerRespS = ptAttribResponder(9, "S Responder",
                                ['State 1', 'State 2', 'State 3',
                                 'State 4', 'State 5', 'State 6', 'SOLVED'])
imagerRespW = ptAttribResponder(10, "W Responder",
                                ['State 1', 'SOLVED', 'State 3',
                                 'State 4', 'State 5', 'State 6', 'State 7'])
imagerRespE = ptAttribResponder(11, "E Responder",
                                ['State 1', 'State 2', 'State 3',
                                 'SOLVED', 'State 5', 'State 6', 'State 7'])
# NPC speech responders and stuff
ImagerOneshot = ptAttribResponder(12, "Resp: Player avatar oneshot")
ShortVisionSound = ptAttribResponder(13, "Resp: Short Vision audio",
                                     ['on', 'off'])
LongVisionSound = ptAttribResponder(14, "Resp: Long Vision audio",
                                    ['on', 'off'])
...
YeeshaSceneTimerDoorClose = ptAttribActivator(39, "AnmEvt: "
                                              "Yeesha scene close door")
...
# globals
PlayFull = 0  # Determines whether full vision will play or not.
              # 0 = partial, 1 = full
...
statesN = ('State 1', 'State 2', 'State 3',
           'State 4', 'SOLVED', 'State 6', 'State 7')
statesS = ('State 1', 'State 2', 'State 3',
           'State 4', 'State 5', 'State 6', 'SOLVED')
statesE = ('State 1', 'State 2', 'State 3',
           'SOLVED', 'State 5', 'State 6', 'State 7')
statesW = ('State 1', 'SOLVED', 'State 3',
           'State 4', 'State 5', 'State 6', 'State 7')
imagerBusted = 0
speechKilled = 0
PuzzleView = 0
...
class clftImager(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        # makes sure Tomahna book is disabled and invisible
        # when linking in to age
        ...
        # gets SDL values of 4 Imager panels,
        # uses this value to retrieve correct state from 4 state lists,
        # and runs the corresponding responder to set
        # initial default state for each state panel
        try:
            intPanelN = self.ageSDL[stringSDLVarPanelN.value][0]
        except:
            intPanelN = 3
            PtDebugPrint("ERROR:  clftImager.OnServerInitComplete():"
                         "\tERROR: age sdl read failed, "
                         "defaulting intPanelN = 3")
        ...
        if boolTomahnaActive:
            PtDebugPrint("clftImager.OnServerInitComplete: "
                         "SDL says Tomahna is active, "
                         "will set Imager to break...")
            imagerBusted = 1
        else:
            PtDebugPrint("clftImager.OnServerInitComplete: "
                         "SDL says Tomahna is NOT active, "
                         "will set Imager to work...")
            imagerBusted = 0

        if boolTomahnaActive and boolSceneYeesha:
            PtDebugPrint("clftImager.OnServerInitComplete(): "
                         "TomahnaActive AND SceneYeesha SDL vars are true, "
                         "will play scene")
            PlayScene = 1
            avatar = 0
            try:
            ...
            PtDebugPrint("clftImager.OnServerInitComplete(): "
                         "TomahnaActive and/or SceneYeesha SDL vars "
                         "are false, no scene for you")
            PlayScene = 0
            # YeeshaName.draw.disable()  ## we're already doing this
                                         ## in clftImager.py
    ...
    def OnNotify(self, state, id, events):
        ...
        if (id == YeeshaSpawner.id):
            ...
            PtDebugPrint("clftImager.OnNotify(): YeeshaName = %s"
                         % (YeeshaName))
            ...
            if PtWasLocallyNotified(self.key):
                for event in events:
                    if (event[0] == kMultiStageEvent and
                        ...
                        PtDebugPrint("clftImager.onNotify: "
                                     "enter puzzle view mode now that "
                                     "seek is done")
                        avatar.draw.disable()
                        # PtFadeLocalAvatar(1)
        ...
        if (id == imagerBrokenBtn.id and state):
            ...
            for event in events:
                if event[0] == 2 and event[1] == 1:  # play avatar oneshot,
        ...
        if (id == ImagerOneshot.id):
            ...
            if windmillRunning == 1 and imagerBusted == 0:
                PtDebugPrint("clftImager.OnNotify: "
                             "SDL says windmill is running, "
                             "so button will do SOMETHING after oneshot...")
                for event in events:
                    if event[0] == 8 and event[1] == 1:  # toggle vision
                        if visionplaying:
                            ...
                            elif (PlayFinal == 0 and
                                  PlayFull == 0 and
                                  PlayTPOT == 0):
                                stopvision = random.randint(
                                    minstoptime, maxstoptime)
                                print ("\tImager will autoshut off "
                                       "in %d seconds" % (stopvision))
                                PtAtTimeCallback(self.key, stopvision, kVision)
                            elif PlayFull == 1:
                                print "nothing"
                                # The full audio of the Yeesha Vision plays
                                # for 137 seconds. This ensures turning it off
                                # after completion.
                                # PtAtTimeCallback(self.key, 141.6, kVision)
            else:
                PtDebugPrint("clftImager.OnNotify: "
                             "SDL says windmill is NOT running, "
                             "so button will stop after oneshot...")
                PtAtTimeCallback(self.key, 1, imagerBtn.id)
        ...
        if (id == GetClothesEvent.id and state):
            ...
            if clothingName not in clothingList:
                print ("adding Yeesha reward clothing %s to wardrobe"
                       % (clothingName))
                avatar.avatar.addWardrobeClothingItem(
                    clothingName, ptColor().white(), ptColor().black())
            else:
                print ("player already has Yeesha reward clothing, "
                       "doing nothing")
        ...
        if (id == YeeshaSceneTimerDoorClose.id and state and PlayScene == 1):
            PtDebugPrint("clftImager.OnNotify(): Yeesha's leaving, "
                         "now shutting Office Door")
            self.ageSDL = PtGetAgeSDL()
        ...
        if (id == YeeshaSceneTimerDone.id and state and PlayScene == 1):
            print ("clftImager.OnNotify(): Yeesha timer is done.  "
                   "Getting rid of Yeesha and setting SDL.")
            self.ageSDL = PtGetAgeSDL()
            YeeshaName.draw.disable()
    ...
    def GetAgeNode(self, age):
        ...
        if not isinstance(chron, type(None)):
            ageChronRefList = chron.getChildNodeRefList()

            if not isinstance(ageChronRefList, type(None)):
    ...
    def EndgameSolved(self):
        ...
        currentStateList.append(
            imagerRespN.state_list.index(imagerRespN.getState()))
        currentStateList.append(
            imagerRespE.state_list.index(imagerRespE.getState()))
        currentStateList.append(
            imagerRespS.state_list.index(imagerRespS.getState()))
        currentStateList.append(
            imagerRespW.state_list.index(imagerRespW.getState()))

        for x in range(4):
            currentStateList[x] = (currentStateList[x]+6) % 7

        print ("clftImager.EndgameSolved(): solution list: " +
               str(solutionList))
        print ("clftImager.EndgameSolved(): currentState list: " +
    ...
    def TPOTSolved(self):
        ...
        print ("clftImager.TPOTSolved(): currentState list  :",
               currentStateList)
                ...
                if folder and folder.folderGetName() == "AgeData":
                    ...
                    for ageDataChildRef in ageDataChildren:
                        ...
                        if chron and chron.getName() == "PelletCaveSolution":
                            ...
                            print ("found pellet cave solution: ",
                                   chronSolutions)
    ...
    def OnTimer(self, id):
        ...
        if visionplaying == 1 and id == kVision:
            print ("\nclftImager.Ontimer:Got kVision timer callback. "
                   "Automatically stopping vision.")
            self.StopVision()
            imagerBrokenBtn.disableActivator()
            imagerLockN.disableActivator()
            imagerLockS.disableActivator()
        ...
        #~ elif visionplaying == 1 and id == kFinished:
            #~ print ("\nclftImager.Ontimer:Got kFinished timer callback. "
            #-        "Automatically stopping vision.")
            #~ finalDone = 1
            #~ self.StopVision()
            #~ imagerBrokenBtn.disableActivator()
            ...
            print ("\nclftImager.Ontimer:Got timer callback. "
                   "Setting up Imager button....")
            imagerBtn.disableActivator()
            ImagerBtnInvisible.run(self.key)
    ...
    def StartVision(self):
        ...
        if PlayFinal == 1:
            ...
            PtDebugPrint("clftImager.StartVision: "
                         "play final Yeesha speech 01, "
                         "then enable Tomahna book...")
            PtAtTimeCallback(self.key, 1, imagerBtn.id)
    ...
    def IDoCityLinksChron(self, agePanel):
        ...
        if not isinstance(entryCityLinks. type(None)):
            valCityLinks = entryCityLinks.chronicleGetValue()
            print "valCityLinks = ", bvalCityLinks
            ...
            if agePanel not in CityLinks:
                ...
                entryCityLinks.save()
                print ("xLinkingBookGUIPopup.IDoCityLinksChron():  "
                       "setting citylinks chron entry to include: ", agePanel)
                valCityLinks = entryCityLinks.chronicleGetValue()
                CityLinks = valCityLinks.split(",")
                print ("xLinkingBookGUIPopup.IDoCityLinksChron():  "
                       "citylinks now = ", CityLinks)
            else:
                print ("xLinkingBookGUIPopup.IDoCityLinksChron():  "
                       "do nothing, citylinks chron already contains: ",
                       agePanel)
        else:
            vault.addChronicleEntry("CityBookLinks", 0, agePanel)
            print ("xLinkingBookGUIPopup.IDoCityLinksChron():  "
                   "creating citylinks chron entry and adding: ", agePanel)
        ...
        if not GotBook:
            ...
            print ("xLinkingBookGUIPopup.IDoCityLinksChron():  "
                   "setting SDL for city book to 1")

    def OnBackdoorMsg(self, target, param):
        if (target == "yeesha1" or
                target == "yeesha2" or
                target == "yeesha3"):
            if param == "off":
                YeeshaName.draw.disable()
                YeeshaName.physics.warpObj(YeeshaWarpHid.value.getKey())
                YeeshaMultiStage.gotoStage(YeeshaName, 0,
                                           dirFlag=1, isForward=1)
            elif param == "on":
                if target == "yeesha1":
                    YeeshaName.draw.enable()
                    YeeshaName.physics.warpObj(YeeshaWarpVis1.value.getKey())
                    YeeshaMultiStage.gotoStage(YeeshaName, 1,
                                               dirFlag=1, isForward=1)
                elif target == "yeesha2":
                    YeeshaName.draw.enable()
                    YeeshaName.physics.warpObj(YeeshaWarpVis2.value.getKey())
                    YeeshaMultiStage.gotoStage(YeeshaName, 2,
                                               dirFlag=1, isForward=1)
                elif target == "yeesha3":
                    YeeshaName.draw.enable()
                    YeeshaName.physics.warpObj(YeeshaWarpVis3.value.getKey())
                    YeeshaMultiStage.gotoStage(YeeshaName, 3,
                                               dirFlag=1, isForward=1)
        elif target == "tpots":
